...
def is_alpha(thing):
	alf = "abcdefghijklmnopqrstuvwxyz ABCDEFGHIJKLMNOPQRSTUVWSYZ"
	for c in thing:
		if c not in alf: return False
	return True
#-------------------------------------------------------------------
# textbox
#-------------------------------------------------------------------
def textbox(message="", title="", text="", codebox=0):
	...
	if codebox:
		...
		character_width = int((root_width * 0.6) / BIG_FONT_SIZE)#SMALL_FONT_SIZE)
		textbox = Text(
			textboxFrame
			, height=25
		...
		textbox.configure(font=(DEFAULT_FONT_FAMILY,BIG_FONT_SIZE))#TEXTBOX_FONT_SIZE))
...
def fileopenbox(msg=None, title=None, startpos=None):
	...
	root = Tk()
	root.withdraw()
	f = tkinter.filedialog.askopenfilename(parent=root,title=title)
	if f == "": return None
	return f
...
def filesavebox(msg=None, title=None, startpos=None):
	...
	root = Tk()
	root.withdraw()
	f = tkinter.filedialog.asksaveasfilename(parent=root, title=title)
	if f == "": return None
	return f
...
tt100.py - pack options: side, expand, fill, anchor """, "\n")
	...
	while 1: # do forever
		...
		"""
		while 1:
			answer = tkSimpleDialog.askstring("What's up", "Name?", initialvalue="Sid")
			if answer == "" or answer == None:
				msgbox(message="Please enter required information")
				continue
			
			if is_alpha(answer):
				break
			else:
				msgbox(message="No specials!")
		print answer
		"""
		...
		if choice == None: 
			...
			break
		while 1:
			answer = enterbox(message="What's Up!", title="Name?")
			if answer == "" or answer == None:
				msgbox(message="Please enter required information")
				continue
		...
		msg2 = "Instructions for Illusion Experiment \n Click OK when you are finished with the instructions"
		f = open("flank.txt", "r")
		t = f.readlines()
		f.close
		textbox(msg2, "Jack's Experiment", t) 
