...
def prepare(blockLab):
    ...
    for blockName in blockLab:
        ...
        for repeat in xrange(config.blockRepeat):
...
def saveTrials(blockList, exp, sub):
    ...
    print >> of, ";","%d/%d/%d; %d:%02d" % (now.month, now.day, now.year, now.hour, now.minute)
    print >> of, ";","Experimenter's name:", exp
    print >> of, ";","Subject's name:", sub
    for line in config.dataFileHeader:
        print >> of, line

    for block in blockList:
        for currTrial in block:
            currTrial.printOut(of)
